...
<head>
    ...
    <title>Artikel Terbaru - Zaydun</title>
    <link rel="icon" href="{{ asset('favicon.ico') }}" type="image/x-icon">
    <meta property="og:site_name" content="Zaydun">
    <meta property="og:type" content="website">
    <meta property="og:url" content="{{ url()->current() }}">
    <meta property="og:title" content="Artikel Terbaru - Zaydun">
    <meta property="og:description" content="Baca artikel edukatif seputar perawatan ikan cupang dan tumbuhan. Tips, panduan, dan inspirasi hobi.">
...
<body class="bg-slate-50 text-gray-800 font-sans flex flex-col min-h-screen justify-between">
    ...
    <main class="max-w-6xl mx-auto px-4 py-10 flex-1 w-full space-y-14">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-800">Artikel</h1>
            <p class="text-sm text-slate-500 mt-1">Kumpulan artikel edukatif seputar ikan cupang dan tumbuhan</p>
        </div>
            ...
            @if($articlesFish->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($articlesFish as $article)
                        <a href="{{ route('articles.show', $article->slug) }}" class="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-md hover:border-cyan-300 transition-all duration-200 group">
                            <img src="{{ asset('uploads/thumbnails/'.$article->thumbnail) }}" alt="{{ $article->title }}" class="w-full h-44 object-cover">
                            <div class="p-4">
            ...
            @if($articlesPlant->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($articlesPlant as $article)
                        <a href="{{ route('articles.show', $article->slug) }}" class="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-md hover:border-emerald-300 transition-all duration-200 group">
                            <img src="{{ asset('uploads/thumbnails/'.$article->thumbnail) }}" alt="{{ $article->title }}" class="w-full h-44 object-cover">
                            <div class="p-4">
