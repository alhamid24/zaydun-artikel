...
<head>
    ...
    <title>Ikan Cupang - Zaydun</title>
    <link rel="icon" href="{{ asset('favicon.ico') }}" type="image/x-icon">
    <meta property="og:site_name" content="Zaydun">
    <meta property="og:type" content="website">
    <meta property="og:url" content="{{ url()->current() }}">
    <meta property="og:title" content="Ikan Cupang - Zaydun">
    <meta property="og:description" content="Inspirasi & kebutuhan untuk Ikan Cupang kesayangan Anda. Tips perawatan, produk berkualitas, dan panduan breeding.">
    ...
    <main class="flex-1 w-full">
        ...
        <div class="max-w-6xl mx-auto px-4 pt-6">
            <nav class="text-xs text-slate-500 flex items-center gap-1.5">
                ...
                <span class="text-cyan-700 font-semibold">🐟 Ikan Cupang</span>
            </nav>
        </div>

        <!-- HERO BANNER -->
        <section class="relative rounded-3xl overflow-hidden shadow-xl max-w-6xl mx-auto px-4 mt-4">
            <div class="absolute inset-0 bg-gradient-to-br from-cyan-600 via-cyan-700 to-sky-900"></div>
            ...
            <div class="relative z-10 py-14 px-6 text-center">
                ...
                <h1 class="text-3xl md:text-5xl font-black text-white mb-4 leading-tight max-w-3xl mx-auto">
                    Inspirasi & Kebutuhan untuk<br>Cupang Kesayangan Anda
                </h1>
                <p class="text-cyan-100 text-base md:text-lg mb-8 max-w-2xl mx-auto leading-relaxed">
                    Temukan tips perawatan, pakan, dan perlengkapan terbaik untuk ikan cupang. Dapatkan semuanya di satu tempat.
                </p>
                <a href="https://wa.me/6281234567890?text=Halo%20Admin%20Zaydun,%20saya%20ingin%20konsultasi%20seputar%20Ikan%20Cupang" target="_blank" class="inline-flex items-center gap-2 bg-white text-cyan-700 font-bold text-sm px-7 py-3 rounded-full hover:bg-cyan-50 shadow-lg transition">
        ...
        <section class="max-w-6xl mx-auto px-4 mt-10">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="bg-white rounded-2xl border border-slate-200 p-5 text-center hover:shadow-md transition">
        ...
        <section class="max-w-6xl mx-auto px-4 mt-10 mb-8">
            <div class="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm p-8 md:p-10">
                <h2 class="text-2xl font-extrabold text-slate-800 mb-6 text-center">🐟 Tentang Ikan Cupang di Zaydun</h2>
                <p class="text-slate-600 leading-relaxed text-sm text-center max-w-3xl mx-auto mb-8">
                    Zaydun hadir untuk para pecinta Ikan Cupang di Indonesia. Kami menyediakan berbagai artikel edukatif — mulai dari tips perawatan harian, panduan breeding, hingga cara mengatasi penyakit cupang. Selain itu, kami juga menyediakan produk-produk pilihan seperti pakan berkualitas, obat-obatan, dan perlengkapan akuarium yang bisa Anda pesan langsung melalui WhatsApp. Semua produk kami pilih secara teliti untuk memastikan yang terbaik untuk cupang kesayangan Anda.
                </p>
                <div class="grid grid-cols-1 md:grid-cols-{{ $admins->count() > 1 ? '2' : '1' }} gap-6 max-w-3xl mx-auto">
                    ...
                    <div class="flex flex-col md:flex-row items-center md:items-start gap-5 p-5 bg-cyan-50/50 rounded-2xl">
                        <div class="shrink-0">
                            @if($admin->photo)
                                <img src="{{ asset('uploads/admins/'.$admin->photo) }}" alt="{{ $admin->name }}" class="w-20 h-20 md:w-24 md:h-24 object-cover rounded-2xl shadow-sm">
                            @else
                        ...
                        <div class="flex-1 text-center md:text-left space-y-2">
                            ...
                            @endif
                            <div class="pt-1">
                                <a href="https://wa.me/{{ $admin->phone ?? '6281234567890' }}?text=Halo%20{{ urlencode($admin->name) }},%20saya%20ingin%20bertanya%20seputar%20Ikan%20Cupang" target="_blank" class="inline-flex items-center gap-1.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs px-4 py-2 rounded-xl transition shadow-sm">
                                    💬 Chat {{ $admin->name }}
                                </a>
                            </div>
                        </div>
        ...
        <section class="max-w-6xl mx-auto px-4 mt-14">
            <div class="flex items-center gap-3 mb-8">
                <div class="w-10 h-10 bg-cyan-100 rounded-xl flex items-center justify-center text-lg">🛒</div>
                <div>
            ...
            @if($products->count() > 0)
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    @foreach($products as $product)
                        <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:border-cyan-400 hover:shadow-lg transition-all duration-300 group">
                            <a href="{{ route('products.show', $product->slug) }}">
                                <div class="aspect-[4/3] bg-slate-100 overflow-hidden">
                                    <img src="{{ asset('uploads/products/'.$product->image) }}" alt="{{ $product->name }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                </div>
                            </a>
                            <div class="p-4 space-y-3">
                                ...
                                <a href="{{ route('products.order', $product->slug) }}" class="block w-full text-center bg-cyan-50 text-cyan-700 text-xs font-bold px-4 py-2.5 rounded-xl border border-cyan-200 hover:bg-cyan-600 hover:text-white hover:border-cyan-600 transition">
                                    💬 Beli via WhatsApp
                                </a>
                            </div>
                        </div>
            ...
            @if($articles->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    @foreach($articles as $article)
                        <a href="{{ route('articles.show', $article->slug) }}" class="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:border-cyan-400 hover:shadow-lg transition-all duration-300 group">
                            <div class="aspect-[16/9] bg-slate-100 overflow-hidden">
                                <img src="{{ asset('uploads/thumbnails/'.$article->thumbnail) }}" alt="{{ $article->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                            </div>
        ...
        <!-- CTA WHATSAPP -->
        <section class="max-w-6xl mx-auto px-4 mt-16 mb-8">
            <div class="relative rounded-3xl overflow-hidden shadow-xl">
                ...
                <div class="relative z-10 py-12 px-6 text-center">
                    ...
                    <a href="https://wa.me/6281234567890?text=Halo%20Admin%20Zaydun,%20saya%20ingin%20bertanya%20seputar%20Ikan%20Cupang" target="_blank" class="inline-flex items-center gap-2 bg-white text-cyan-700 font-bold text-sm px-7 py-3 rounded-full hover:bg-cyan-50 shadow-lg transition">
