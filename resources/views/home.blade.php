...
<body class="bg-slate-50 text-gray-800 font-sans flex flex-col min-h-screen justify-between">
    ...
    <main class="max-w-6xl mx-auto px-4 py-8 space-y-16 flex-1 w-full">
        ...
        <section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
                ...
                <!-- Card Ikan Cupang -->
                <a href="{{ route('kategori.ikan') }}" class="group bg-gradient-to-br from-cyan-50 to-white border-2 border-cyan-200 rounded-3xl p-8 text-center flex flex-col items-center justify-between hover:border-cyan-500 hover:shadow-lg transition-all duration-300">
                ...
                <!-- Card Tumbuhan -->
                <a href="{{ route('kategori.tumbuhan') }}" class="group bg-gradient-to-br from-emerald-50 to-white border-2 border-emerald-200 rounded-3xl p-8 text-center flex flex-col items-center justify-between hover:border-emerald-500 hover:shadow-lg transition-all duration-300">
        ...
        <section id="ikan-cupang" class="pt-8">
            ...
            <div class="mb-8">
                ...
                @if($productsFish->count() > 0)
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                        @foreach($productsFish as $product)
                ...
                @else
                    <div class="p-8 text-center text-sm text-slate-400 bg-white rounded-2xl border-2 border-dashed border-slate-200">Belum ada produk Ikan Cupang.</div>
                @endif
            </div>
            ...
            <div class="space-y-4">
                ...
                @forelse($articlesFish as $article)
                    <a href="{{ route('articles.show', $article->slug) }}" class="bg-white p-4 rounded-2xl border border-slate-200 flex gap-4 items-center hover:border-cyan-400 hover:shadow-md transition-all duration-200 group">
                        <img src="{{ asset('uploads/thumbnails/'.$article->thumbnail) }}" alt="{{ $article->title }}" class="w-20 h-20 object-cover rounded-xl shrink-0 bg-slate-100">
        ...
        <section id="tumbuhan" class="pt-8 border-t border-slate-200">
            ...
            <div class="mb-8">
                ...
                @if($productsPlant->count() > 0)
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                        @foreach($productsPlant as $product)
                            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:border-emerald-400 hover:shadow-lg transition-all duration-300 group">
                                <a href="{{ route('products.show', $product->slug) }}">
                                    <div class="aspect-[4/3] bg-slate-100 overflow-hidden">
                                        <img src="{{ asset('uploads/products/'.$product->image) }}" alt="{{ $product->name }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                    </div>
                                ...
                                <div class="p-4 space-y-3">
                                    ...
                                    <a href="{{ route('products.order', $product->slug) }}" class="block w-full text-center bg-emerald-50 text-emerald-700 text-xs font-bold px-4 py-2.5 rounded-xl border border-emerald-200 hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition">
                                        💬 Beli via WhatsApp
                                    </a>
                                </div>
                            </div>
                ...
                @else
                    <div class="p-8 text-center text-sm text-slate-400 bg-white rounded-2xl border-2 border-dashed border-slate-200">Belum ada produk Tumbuhan.</div>
                @endif
            </div>
            ...
            <div class="space-y-4">
                ...
                @forelse($articlesPlant as $article)
                    <a href="{{ route('articles.show', $article->slug) }}" class="bg-white p-4 rounded-2xl border border-slate-200 flex gap-4 items-center hover:border-emerald-400 hover:shadow-md transition-all duration-200 group">
                        <img src="{{ asset('uploads/thumbnails/'.$article->thumbnail) }}" alt="{{ $article->title }}" class="w-20 h-20 object-cover rounded-xl shrink-0 bg-slate-100">
                        <div class="flex-1 min-w-0">
        ...
        <section class="max-w-6xl mx-auto px-4 mt-16 mb-8">
            <div class="relative rounded-3xl overflow-hidden shadow-xl">
                ...
                <div class="relative z-10 py-12 px-6 text-center">
                    ...
                    <a href="https://wa.me/6281234567890" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 bg-white text-emerald-700 font-bold text-sm px-7 py-3 rounded-full hover:bg-emerald-50 shadow-lg transition">
                        💬 Pesan via WhatsApp →
                    </a>
                </div>
            </div>
    ...
    <!-- FLOATING WA BUTTON -->
    <a href="https://wa.me/6281234567890" target="_blank" rel="noopener noreferrer" class="fixed bottom-6 right-6 bg-emerald-500 hover:bg-emerald-600 text-white p-4 rounded-full shadow-xl flex items-center justify-center transition duration-300 hover:scale-110 z-50" title="Pesan via WhatsApp">
