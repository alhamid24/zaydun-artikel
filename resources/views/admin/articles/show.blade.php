...
<body class="bg-slate-50 text-gray-800 font-sans">
    ...
    <nav class="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50">
        <div class="max-w-6xl mx-auto px-4 h-16 flex justify-between items-center">
            <a href="{{ url('/') }}" class="text-2xl font-extrabold tracking-wider text-teal-600 transition-transform duration-300 hover:scale-105">ZAYDUN</a>
    ...
    <!-- ARTICLE CONTENT -->
    <article class="max-w-3xl mx-auto px-4 py-10 reveal">
    ...
    <section class="bg-white border-t border-slate-200 py-12 mt-12 reveal">
        <div class="max-w-6xl mx-auto px-4">
            <h3 class="text-xl font-bold text-slate-800 mb-6">Artikel Terkait Lainnya</h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                @foreach($relatedArticles as $related)
                    <a href="{{ route('articles.show', $related->slug) }}" class="z-card bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md hover:border-teal-300 transition-all duration-200 block group">
                        <img loading="lazy" decoding="async" src="{{ asset('uploads/thumbnails/'.$related->thumbnail) }}" class="w-full h-40 object-cover">
                        <div class="p-4">
