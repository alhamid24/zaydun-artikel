...
<div class="max-w-3xl bg-white p-8 rounded-2xl shadow-sm border border-gray-200">
    <div class="mb-6 flex justify-between items-start">
        <div>
            ...
            <p class="text-sm text-gray-500">Perbarui informasi artikel Zaydun.</p>
        </div>
    ...
    <form action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title" id="article_title" value="{{ old('title', $article->title) }}" placeholder="Contoh: Tips Merawat Tanaman Hias untuk Pemula" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500" required>
        </div>
        ...
        <div>
            ...
            <select name="tag" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500">
                ...
                <optgroup label="Ikan">
                    <option value="pembenihan-ikan" {{ old('tag', $article->tag) == 'pembenihan-ikan' ? 'selected' : '' }}>Pembenihan Ikan</option>
                    <option value="pembersihan-ikan" {{ old('tag', $article->tag) == 'pembersihan-ikan' ? 'selected' : '' }}>Pembersihan Ikan</option>
                    <option value="penyakit-ikan" {{ old('tag', $article->tag) == 'penyakit-ikan' ? 'selected' : '' }}>Penyakit Ikan</option>
                </optgroup>
                <optgroup label="Tumbuhan">
                    <option value="penanaman" {{ old('tag', $article->tag) == 'penanaman' ? 'selected' : '' }}>Penanaman</option>
                    <option value="perawatan-tanaman" {{ old('tag', $article->tag) == 'perawatan-tanaman' ? 'selected' : '' }}>Perawatan Tanaman</option>
                    <option value="hama-penyakit" {{ old('tag', $article->tag) == 'hama-penyakit' ? 'selected' : '' }}>Hama & Penyakit</option>
                </optgroup>
            </select>
        </div>
        ...
        <div>
            ...
            @if($article->thumbnail)
                <div class="mb-3">
                    <img loading="lazy" decoding="async" src="{{ asset('uploads/thumbnails/'.$article->thumbnail) }}" alt="Thumbnail saat ini" class="w-48 h-28 object-cover rounded-xl border border-gray-200">
                    <p class="text-xs text-gray-400 mt-1">Thumbnail saat ini. Upload gambar baru di bawah jika ingin mengganti.</p>
                </div>
            @endif
            <input type="file" name="thumbnail" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100">
        ...
        <div>
            ...
            <textarea name="content" id="article_content" rows="8" placeholder="Tulis artikel lengkap di sini..." class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500" required>{{ old('content', $article->content) }}</textarea>
        </div>
        ...
        <div class="bg-slate-50 border border-gray-200 rounded-2xl p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $article->meta_title) }}" maxlength="70" placeholder="Otomatis dari judul artikel..." class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500" oninput="updateSeoPreview()">
            ...
            <div>
                ...
                <textarea name="meta_description" id="meta_description" rows="3" maxlength="160" placeholder="Otomatis dari isi artikel..." class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500" oninput="updateSeoPreview()">{{ old('meta_description', $article->meta_description) }}</textarea>
                <div class="flex justify-between mt-1">
            ...
            <div>
                ...
                <input type="text" name="meta_keywords" value="{{ old('meta_keywords', $article->meta_keywords) }}" placeholder="ikan hias, aquascape, merawat tanaman" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500">
        ...
        <div class="flex items-center">
            <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', $article->is_published) ? 'checked' : '' }} class="w-4 h-4 text-emerald-600 border-gray-300 rounded focus:ring-emerald-500">
