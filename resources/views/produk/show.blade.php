...
<body class="bg-slate-50 text-gray-800 font-sans flex flex-col min-h-screen justify-between">
    ...
    <main class="flex-1 w-full">
            ...
            <div class="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-0">
                    ...
                    <div class="bg-slate-100 flex items-center justify-center p-8">
                        <img src="{{ asset('uploads/products/'.$product->image) }}" alt="{{ $product->name }}" class="w-full max-w-md object-cover rounded-2xl">
                    </div>
                    ...
                    <div class="p-6 md:p-10 flex flex-col justify-center space-y-5">
                        ...
                        <div class="pt-4 space-y-3">
                            <a href="{{ route('products.order', $product->slug) }}" class="inline-flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-sm px-8 py-3.5 rounded-xl transition shadow-sm w-full justify-center">
        ...
        <section class="max-w-6xl mx-auto px-4 mt-14 mb-10">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                @foreach($relatedProducts as $related)
                    <a href="{{ route('products.show', $related->slug) }}" class="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-lg transition-all duration-300 group">
                        <div class="aspect-[4/3] bg-slate-100 overflow-hidden">
                            <img src="{{ asset('uploads/products/'.$related->image) }}" alt="{{ $related->name }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                        </div>
