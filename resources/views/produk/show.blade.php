            ...
            <div class="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-0">
                    ...
                    <div class="p-6 md:p-10 flex flex-col justify-center space-y-5">
                        ...
                        @if($product->stock > 0)
                            <div class="inline-flex items-center gap-2 text-xs font-bold rounded-full px-3 py-1.5 {{ $product->stock <= 5 ? 'bg-amber-50 text-amber-700 border border-amber-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200' }}">
                                <span class="w-2 h-2 rounded-full {{ $product->stock <= 5 ? 'bg-amber-500' : 'bg-emerald-500' }}"></span>
                                {{ $product->stock <= 5 ? 'Sisa '.$product->stock.' unit' : 'Stok tersedia' }}
                            </div>
                        @else
                            <div class="inline-flex items-center gap-2 text-xs font-bold rounded-full px-3 py-1.5 bg-red-50 text-red-600 border border-red-200">
                        ...
                        <div class="pt-4 space-y-3">
                            ...
                            @else
                                <button type="button" disabled class="z-btn inline-flex items-center gap-2 bg-slate-200 text-slate-400 font-bold text-sm px-8 py-3.5 rounded-xl w-full justify-center cursor-not-allowed">
                                    <x-icon name="wa" class="w-4 h-4 inline-block -mt-0.5 align-middle" /> Stok Habis
                                </button>
                                <p class="text-xs text-slate-400 text-center">Produk sedang tidak tersedia</p>
                            @endif
                        </div>
                    </div>
                </div>
        ...
        <section class="max-w-6xl mx-auto px-4 mt-14 mb-10 reveal">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                @foreach($relatedProducts as $related)
                    <a href="{{ route('products.show', $related->slug) }}" class="z-card relative bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-lg transition-all duration-300 group">
                        <x-stock-badge :product="$related" />
