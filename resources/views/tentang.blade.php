...
<body class="bg-slate-50 text-gray-800 font-sans flex flex-col min-h-screen justify-between">
    ...
    <main class="max-w-4xl mx-auto px-4 py-16 flex-1 w-full">
        ...
        <!-- PROFIL PEMILIK -->
        @if($owner)
        <div class="bg-white rounded-3xl border border-slate-200 p-8 md:p-12 mb-8">
            <div class="flex flex-col md:flex-row items-center md:items-start gap-8">
                <div class="shrink-0">
                    @if($owner->photo)
                        <img src="{{ asset('uploads/owner/'.$owner->photo) }}" alt="{{ $owner->name }}" class="w-32 h-32 md:w-40 md:h-40 object-cover rounded-3xl shadow-lg">
                    @else
                ...
                <div class="flex-1 text-center md:text-left space-y-4">
                    ...
                    @endif
                    <div class="pt-2">
                        <a href="https://wa.me/6281234567890?text=Halo%20{{ urlencode($owner->name) }},%20saya%20ingin%20bertanya%20seputar%20Zaydun" target="_blank" class="inline-flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-sm px-6 py-3 rounded-xl transition shadow-sm">
        ...
        <!-- TENTANG ZAYDUN -->
        <div class="bg-white rounded-3xl border border-slate-200 p-8 md:p-12 space-y-6">
